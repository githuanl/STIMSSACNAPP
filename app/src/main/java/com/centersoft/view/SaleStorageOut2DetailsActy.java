...
public class SaleStorageOut2DetailsActy extends BaseActivity {
    ...
    protected void setToolBarTitle() {
        toobarTitle.setText("销售出库明细");
    }

    @Override
    protected int initResource() {
        return R.layout.acty_salestorage_details;
    }
    ...
    protected void initData() {
        ...
        // 客户名称
        if (!TextUtils.isEmpty(stroageOutDetail.getCustid())) {
            NetCashUtil.getBaseCash(CashType.Customer, stroageOutDetail.getCustid(), new NetCashUtil.CallBack<String>() {
                @Override
                public void success(String result) {
                    custid.setText(result);
                }
            });
        }
        // 仓区库位
            ...
            httpService.getMaterialByBarcode(barcodeStr)
                    ...
                    .subscribe(new BaseObserver<BaseEntity<StroageOut>>(dialog) {
                        ...
                        protected void onSuccess(BaseEntity<StroageOut> baseEntity) {
                            StroageOut stroageOut = baseEntity.getData();
                            String  manubatchnum = stroageOut.getManubatchnum();
                            if (bachNums.contains(manubatchnum)) {
                                bachNums.remove(manubatchnum);
                            } else {
                                if (stroageOutDetail.getBatchnum().contains(stroageOut.getManubatchnum())) {
        ...
        switch (v.getId()) {
            case R.id.bt_confirm:
                ...
                if (stroageOutDetail.getBatchnum() != null && bachNums.size() > 0) {
                    showTips(R.drawable.jd_toast_jg, "请分别扫描指定的批次");
                    return;
                }
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface d, int which) {
                        ...
                        httpService.confirmDeliverByScan(reqMap)
                                .compose(new CommonTransformer())
